...
App.prototype.tileWindows = function () {
  ...
  windows.forEach(win => {
    // Add random jitter (30-40 pixels in either positive or negative direction)
    const jitterX = Math.random() < 0.5 ? randomJitter(-40, -30) : randomJitter(30, 40);
    const jitterY = Math.random() < 0.5 ? randomJitter(-40, -30) : randomJitter(30, 40);

    win.to(positions[counter], offsetX + jitterX, offsetY + jitterY);
...
Key.on('m', controlAltCommand, function () {
  ...
  const secondaryFrame = secondaryScreen.flippedVisibleFrame();

  // Move all windows to secondary display
  windows.forEach(win => {
    if (win.screen().identifier() !== secondaryScreen.identifier()) {
      win.setFrame({
        x: secondaryFrame.x + margin,
        y: secondaryFrame.y + margin,
